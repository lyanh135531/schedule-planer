...
import { User, Lock, ArrowRight, Sparkles, ShieldCheck, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
	...
	return (
		<div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#0f1117]">
			...
			<div className="relative z-10 w-full max-w-[440px] px-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
				...
				<div className="glass-panel rounded-md p-1 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
					{/* Inner Card Glow */}
					...
					<div className="relative bg-[#161923]/60 backdrop-blur-3xl rounded-md p-6 md:p-10 border border-white/5 space-y-6 md:space-y-8">
						...
						<form onSubmit={handleLogin} className="space-y-6">
								...
								<div className="space-y-2">
									...
									<div className="relative group">
										...
										<button
											...
											className="absolute cursor-pointer right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-gray-500 hover:text-orange-500 transition-all duration-300 focus:outline-none rounded-full hover:bg-orange-500/10 active:scale-90 group/toggle"
										>
											{showPassword ? (
												<EyeOff className="w-4 h-4 transition-transform duration-300 group-hover/toggle:scale-110" />
											) : (
												<Eye className="w-4 h-4 transition-transform duration-300 group-hover/toggle:scale-110" />
											)}
										</button>
									</div>
								</div>
							...
							{error && (
								<div className="p-3 rounded-md bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold text-center animate-shake">
									{error}
								</div>
							)}
